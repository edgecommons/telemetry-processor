...
- **Initialization gate.** The script does not run until every input marked `required` (the default)
  has been observed at least once — a half-initialized OEE is never computed. An input with
  `"required": false` doesn't gate; it is simply absent from `inputs` until it arrives, so the
  script checks for it (`inputs.x != ()` in Rhai, `inputs.x ~= nil` in Lua… or Rhai's `"x" in inputs`).
...
- **Restart-empty.** The cache is in-memory; after a restart the stage deterministically re-awaits
  its required inputs before the first evaluation (each initialization is logged).
...
A working OEE script over those inputs:
